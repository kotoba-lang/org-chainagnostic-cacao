...
;; interpreter does?
;;
;; `window_kotoba_test.clj` drives the guest through `kotoba.kir`, which
  ...
  (delay
    (let [{:keys [kotoba nbb runtime]} @tools]
      (if-not (and kotoba nbb runtime)
        ...
        (let [r (shell/sh nbb "test/siwe/artifact_probe.cljs" kotoba amu-root)
...
(deftest the-native-backend-refuses-this-guest-and-says-why
  ...
  ;; native backends today -- not Wasm, and not anything about EIP-4361.
  ...
  (if-let [bin (:kotoba @tools)]
    (let [out (str (System/getProperty "java.io.tmpdir") "/siwe-window-native.kexe")
